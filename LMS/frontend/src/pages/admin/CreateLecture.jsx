...
function CreateLecture() {
    ...
    const createLectureHandler = async () => {
      ...
      try {
        ...
        dispatch(setLectureData([...(lectureData || []), result.data.lecture])) // ✅ fixed
    ...
    useEffect(()=>{
      const getLecture = async () => {
        try {
          ...
          console.log(result.data)
          dispatch(setLectureData(result.data.lectures || result.data || [])) // ✅ fixed
        } catch (error) {
           console.log(error)
           toast.error(error?.response?.data?.message || "Failed to load lectures") // ✅ fixed
        }
      }
      getLecture()
    },[])
     ...
     <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
      <div className="bg-white shadow-xl rounded-xl w-full max-w-2xl p-6">
        ...
        <div className="space-y-2">
          {lectureData?.map((lecture, index) => ( // ✅ fixed
